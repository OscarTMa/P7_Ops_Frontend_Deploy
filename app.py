...
st.sidebar.header("**Informations Générales du Tableau de Bord**")
st.sidebar.markdown("Nombre de demandes de prêt dans l'échantillon :", unsafe_allow_html=True)
st.sidebar.text(10000)

# Choisissez un nombre maximum de fichiers de données du demandeur à analyser
nb_files = st.sidebar.slider("Choisissez un nombre maximal de fichiers de données de demandeur à analyser", 500, 9999, 500)

num_neighbors = st.sidebar.slider("Choisissez un nombre de fichiers de demandeurs similaires au fichier en cours d'analyse", 10, 15, 10)
...
# CALCUL DE SIMILARITÉ PAR LA MÉTHODE DE LA DISTANCE EUCLIDIENNE KNN #
...
#Construisons une liste de dataframes pour les voisins standards de sigle
...
#FIN DE CALCUL DE SIMILARITÉ PAR LA MÉTHODE DE DISTANCE EUCLIDIENNE KNN
def main() : 
       
    #@st.cache(allow_output_mutation=True)
    #@st.cache_data()
    #@st.cache(allow_output_mutation=True)   # 1 - 6
    ...
    #@st.cache(allow_output_mutation=True) # 2 - 6
    #@st.cache_resource(persist=True)
    @st.cache_data
    ...
    #@st.cache(allow_output_mutation=True) # 3 - 6
    #@st.cache_resource(persist=True)
    @st.cache_data
    ...
    #@st.cache(allow_output_mutation=True) # 4 - 6
    #@st.cache_data()
    #@st.cache_resource(persist=True)
    @st.cache_data
...
# Cette fonction envoie x à l'API et récupère la prédiction
#     
    #@st.cache(allow_output_mutation=True)  # 6 - 6
    #@st.cache_resource()
    #@st.cache_resource(persist=True)
    @st.cache_data
...
# 3 SIDEBAR DATA
    ...
    #Average income
    st.sidebar.markdown("Revenu moyen des demandeurs (USD) :", unsafe_allow_html=True)
    st.sidebar.text(rev_moy)   
    # 4 DONNÉES DE LA BARRE LATÉRALE
    ...
    # 1 TITRE D'AFFICHAGE DES APPLICATIONS
    ...
    # <h3 style="color: black; text-align:center">**Credit Risk Project implemented **</h3>
    ...
    # 1 Afficher l'ID de l'emprunteur depuis la barre latérale
  ...
  # 2 Affichage des informations sur le demandeur : 
  # Genre du demandeur, Âge, Situation familiale, Enfants
...
# PRÉDICTION
# 3 Affichage de la solvabilité du demandeur
  
    st.header("**Analyse du fichier du demandeur**")
    prediction = load_prediction(data, idx_clt)
    st.write("La probabilité de défaut est de {:.2f}%".format(round(prediction*100), 2))
...
# 4 Importance des caractéristiques / description
    ...
    if st.checkbox("Need help about Applicant feature description ?") :
         ...
         st.markdown("<i>…</i>", unsafe_allow_html=True)
# Afficher les N fichiers similaires au fichier actuel   
